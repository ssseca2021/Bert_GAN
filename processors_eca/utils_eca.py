...
class DataProcessor(object):
    ...
    def _read_ch_pkl(self, data_path, save_csv_path = None):
        ...
        for index, item in enumerate(data):
            ...
            content_data = [] #里面的每个元素表示的是没个子句的list
            clause_len = [] #每个子句的长度
            span_index = [] #每个元素表示 原因在当前子句中开始和结束的位置
            ...
            for indexc, itemc in enumerate(clause_info):
                ...
                if ifcause == 'Y':
                    ...
                    if end > max_cause_len:
                        remove_len= end - max_cause_len + 1
                        start  = max(0, start - remove_len)
                        end = end - remove_len
                        content_l = content_l[remove_len:]
                    span_index.append([start, end - 1])
                    a = 1
                else:
                    span_index.append([-1, -1])
                
                # content_l.append('[SEP]')#添加【SEP】字符
                content_data.append(content_l) #添加子句的word
                clause_len.append(len(content_l))#获取子句的长度
            ...
            # example_dic['target_data'] = target_data
            example_dic['clause_len'] = clause_len
            example_dic['clause_num'] = len(content_data)
            example_dic['span_index'] = span_index
            return_data.append(example_dic)
        # for i in range(0, 3):
        #     dd = return_data[i]
